...
void a3demo_initScene(a3_DemoState *demoState)
{
	...
	demoState->displayPhysicsText = 1;
	...
	// object scales
	a3real4x4SetScale(demoState->groundScale.m, 24.0f);
	a3real4x4SetScale(demoState->rampScale.m, 10.0f);
	a3real4x4SetScale(demoState->anchorScale.m, 1.0f);
	a3real4x4SetScale(demoState->particleScale.m, 0.1f);
}
...
void a3demo_update(a3_DemoState *demoState, double dt)
{
	...
	if (a3physicsLockWorld(demoState->physicsWorld) > 0)
	{
		...
		// copy other
		demoState->groundNormal = demoState->physicsWorld->groundNormal;
		demoState->rampStationaryNormal = demoState->physicsWorld->rampStationaryNormal;
		demoState->rampTiltingNormal = demoState->physicsWorld->rampTiltingNormal;
		demoState->rampRotatingNormal = demoState->physicsWorld->rampRotatingNormal;
	
		// unlock
		a3physicsUnlockWorld(demoState->physicsWorld);

		// copy from physics world to demo state here
		for (i = 0; i < demoState->physicsWorld->particlesActive; ++i)
		{
			// positions
			demoState->physicsObjects[i].position =
				worldState->position[i].xyz;
	...
	a3real4x4ConcatL(demoState->groundObject->modelMat.m, demoState->groundScale.m);
	a3real4x4ConcatL(demoState->springAnchorObject->modelMat.m, demoState->anchorScale.m);
	a3real4x4ConcatL(demoState->springAnchorOtherObject->modelMat.m, demoState->anchorScale.m);
	a3real4x4ConcatL(demoState->rampStationaryObject->modelMat.m, demoState->rampScale.m);
	a3real4x4ConcatL(demoState->rampTiltingObject->modelMat.m, demoState->rampScale.m);
	a3real4x4ConcatL(demoState->rampRotatingObject->modelMat.m, demoState->rampScale.m);

	// particles
	for (i = 6, tmpObject = demoState->physicsObjects + i; i < demoState->physicsWorld->particlesActive; ++i, ++tmpObject)
		a3real4x4ConcatL(tmpObject->modelMat.m, demoState->particleScale.m);
}
...
*/
	...
	currentSceneObject = demoState->rampStationaryObject;
	modelMat = currentSceneObject->modelMat;
	a3real4x4Product(modelViewProjectionMat.m, demoState->camera->viewProjectionMat.m, modelMat.m);
	a3shaderUniformSendFloatMat(a3unif_mat4, 0, currentDemoProgram->uMVP, 1, modelViewProjectionMat.mm);
	a3shaderUniformSendFloat(a3unif_vec4, currentDemoProgram->uColor, 1, grey);
	...
	currentSceneObject = demoState->rampTiltingObject;
	modelMat = currentSceneObject->modelMat;
	a3real4x4Product(modelViewProjectionMat.m, demoState->camera->viewProjectionMat.m, modelMat.m);
	a3shaderUniformSendFloatMat(a3unif_mat4, 0, currentDemoProgram->uMVP, 1, modelViewProjectionMat.mm);
	a3shaderUniformSendFloat(a3unif_vec4, currentDemoProgram->uColor, 1, grey);
	...
	currentSceneObject = demoState->rampRotatingObject;
	modelMat = currentSceneObject->modelMat;
	a3real4x4Product(modelViewProjectionMat.m, demoState->camera->viewProjectionMat.m, modelMat.m);
	a3shaderUniformSendFloatMat(a3unif_mat4, 0, currentDemoProgram->uMVP, 1, modelViewProjectionMat.mm);
	a3shaderUniformSendFloat(a3unif_vec4, currentDemoProgram->uColor, 1, grey);
	...
	currentSceneObject = demoState->springAnchorObject;
	modelMat = currentSceneObject->modelMat;
	a3real4x4Product(modelViewProjectionMat.m, demoState->camera->viewProjectionMat.m, modelMat.m);
	a3shaderUniformSendFloatMat(a3unif_mat4, 0, currentDemoProgram->uMVP, 1, modelViewProjectionMat.mm);
	...
	currentSceneObject = demoState->springAnchorOtherObject;
	modelMat = currentSceneObject->modelMat;
	a3real4x4Product(modelViewProjectionMat.m, demoState->camera->viewProjectionMat.m, modelMat.m);
	a3shaderUniformSendFloatMat(a3unif_mat4, 0, currentDemoProgram->uMVP, 1, modelViewProjectionMat.mm);
	a3shaderUniformSendFloat(a3unif_vec4, currentDemoProgram->uColor, 1, orange);
	...
	// sphere: moving particles
	currentDrawable = demoState->draw_sphere;
	a3vertexActivateDrawable(currentDrawable);
	for (i = 6; i < demoState->physicsWorld->particlesActive; ++i)
	{
		currentSceneObject = demoState->physicsObjects + i;
		modelMatOrig = modelMat = currentSceneObject->modelMat;
		a3real4x4TransformInverseIgnoreScale(modelMatInv.m, modelMat.m);
		a3real4x4Product(modelViewProjectionMat.m, demoState->camera->viewProjectionMat.m, modelMat.m);
		a3shaderUniformSendFloatMat(a3unif_mat4, 0, currentDemoProgram->uMVP, 1, modelViewProjectionMat.mm);
		a3shaderUniformSendFloat(a3unif_vec4, currentDemoProgram->uColor, 1, particleColor[i % 6]);
		a3vertexRenderActiveDrawable();
	}
...
*/
	...
	if (demoState->showText && demoState->textInit && demoState->displayPhysicsText)
	{
		...
		for (i = 0; i < demoState->physicsWorld->particlesActive; ++i)
		{
			...
			pos_model.xyz = demoState->physicsObjects[i].position;
			...
			// display text
			a3textDraw(demoState->text, pos_ndc.x, pos_ndc.y, pos_ndc.z, 1.0f, 1.0f, 1.0f, 1.0f,
				objectDescriptions[i]);
		}
		glEnable(GL_DEPTH_TEST);
	}
